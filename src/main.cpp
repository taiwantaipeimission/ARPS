...
bool save(ReportCollection* report_collection, CompList* comp_list, MessageHandler* message_handler, std::wstring date, std::wstring english_date, FileManager* file_manager)
{
	report_collection->total_all(comp_list, date, english_date);
	report_collection->save_all();
	message_handler->save(file_manager);

	return true;

}

bool load(ReportCollection* report_collection, CompList* comp_list, MessageHandler* message_handler, FieldCollection* config, FileManager* file_manager)
{
	report_collection->load_all();
	comp_list->load(file_manager);
	message_handler->load(file_manager);
...
void save_cb(Fl_Widget* wg, void* ptr)
{
	...
	save(terminal->report_collection, terminal->comp_list, terminal->msg_handler, terminal->date, terminal->english_date, &file_manager);
	saved = true;
}
...
void process_msg_cb(Fl_Widget* wg, void* ptr)
{
	...
	for (int j = 1; j <= unhandled->size(); j++)
	{
		...
		{
			terminal->msg_handler->process_msg((Message*)unhandled->data(j), terminal, terminal->report_collection, terminal->comp_list, &referral_file, terminal->date, terminal->english_date);
		}
	}
	run_terminal_commands(terminal);
	update_msg_scroll(terminal->msg_handler);
